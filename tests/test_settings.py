...
def test_settings_page(client):
    ...
    assert response.status_code == 200 and b"Download backup" in response.data
...
def test_backup_contains_database_and_photos(client, app):
    ...
    assert "tracker.sqlite" in zf.namelist() and "photos/animals/1.jpg" in zf.namelist()
...
def test_restore_replaces_data(client, app):
    ...
    _add_pet(client, "Rex")
    response = client.post("/settings/restore", data={"backup": (io.BytesIO(backup), "b.zip"), "confirm": "yes"},
                           content_type="multipart/form-data", follow_redirects=True)
    assert b"Backup restored. 1 pet loaded." in response.data
    with app.app_context():
        assert [a.name for a in models.list_animals()] == ["Mochi"]


def test_restore_requires_confirmation_and_rejects_bad_files(client, app):
    _add_pet(client)
    backup = client.get("/settings/backup.zip").data
    response = client.post("/settings/restore", data={"backup": (io.BytesIO(backup), "b.zip")},
                           content_type="multipart/form-data", follow_redirects=True)
    assert b"tick the box" in response.data
    for payload, message in ((b"not a zip", b"not a zip backup"), (_zip({"readme.txt": b"hi"}), b"does not contain"),
                             (_zip({"tracker.sqlite": b"x", "../evil": b"x"}), b"unsafe paths")):
        response = client.post("/settings/restore", data={"backup": (io.BytesIO(payload), "b.zip"), "confirm": "yes"},
                               content_type="multipart/form-data", follow_redirects=True)
...
def test_secret_key_is_generated_and_reused(app):
    ...
    again = create_app({"TESTING": True, "DATA_DIR": app.config["DATA_DIR"], "DATABASE": app.config["DATABASE"],
